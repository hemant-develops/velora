...
export const MessagesScreen: React.FC = () => {
  ...
  const { user } = useAuth();
  const { getConversationsForUser, isLoaded } = useMessages();
  ...
  const conversations = getConversationsForUser(user.id, user.role);
  ...
  return (
    <FlatList
      ...
      ListHeaderComponent={<Text style={[typography.displayMd, { marginBottom: spacing.lg }]}>Messages</Text>}
      renderItem={({ item }) => {
        const partner = partnerOf(item);
        const unread = unreadOf(item);
        return (
          <Pressable style={styles.row} onPress={() => navigation.navigate('ConversationDetail', { conversationId: item.id })}>
